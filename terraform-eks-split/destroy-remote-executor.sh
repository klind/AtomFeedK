...
echo "🔄 Starting destruction process via remote executor..."
...
cleanup_kubernetes_resources() {
    ...
    local VPC_ID
    VPC_ID=$(./terraform-remote-executor.sh "." "output vpc_id" 2>/dev/null || echo "")
...
if "./terraform-remote-executor.sh" "." "state list" | grep -q "kubernetes_config_map.aws_auth"; then
    echo "🔄 Removing aws_auth ConfigMap from Terraform state"
    "./terraform-remote-executor.sh" "." "state rm kubernetes_config_map.aws_auth"
else
